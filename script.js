const CONFIG = {
  ...
  noEscapeTexts: [
    "Eits, ga bisa😝",
    "Hehe, coba lagi 😜",
    "Yaudah deh, aku anggap YES ya! 💕",
  ],
  confirmTitle: "YEAAAY! Km Beneran Mau 💗",
  confirmText: "Mulai dari sekarang km resmi jadi…",
  badgeText: "My Valentine hehe >_<",
  nextButton: "Lanjut ",
  ...
  finalMessage: `Makasii yaa udaa hadir, walaupun kita sekarang bukan siapa-siapa, tapi ga menutup kemungkinan buat selalu deket, saling bantu, dan sharing cerita satu sama lainn, dan aku juga gatau mau bilang ini ke siapa lagii, karna disini cuma adaa km hehe, semoga hari hari km selalu di iringi sama hal hal baik yaa, semoga km bahagia selalu dan bisa meraih apa yang km impikann, ku berharap kita ga asing yaa, apapun badai yang datang semoga kita bisa hadapi nya bareng-barengg hehe.. Happy Valentine 💕`,
...
function handleNoClick() {
  ...
  if (state.noClickCount <= 2) {
    elements.noEscapeText.textContent =
      CONFIG.noEscapeTexts[state.noClickCount - 1];
...
function applyConfig() {
  ...
  elements.nextToPage3.querySelector(".btn-text").textContent =
    CONFIG.nextButton;
  elements.backToPage1.textContent = CONFIG.repeatButton;
  document.getElementById("flowerTitle").textContent = CONFIG.flowerTitle;
  elements.takeBouquet.querySelector(".btn-text").textContent =
    CONFIG.takeBouquetButton;
  elements.bouquetMessage.textContent = CONFIG.bouquetMessage;
  elements.nextToPage4.querySelector(".btn-text").textContent =
    CONFIG.nextFlowerButton;
  document.getElementById("collageTitle").textContent = CONFIG.collageTitle;
  document.getElementById("collageSubtitle").textContent =
    CONFIG.collageSubtitle;
  elements.nextToPage5.querySelector(".btn-text").textContent =
    CONFIG.nextCollageButton;
  ...
  elements.restartBtn.querySelector(".btn-text").textContent =
